...
window.methods = {
    ...
    new_tab = function (w, arg, opts)
        ...
        local win = window.new({arg})
        return win.view
    end,

    new_tab_orig = function (w, arg, opts)
        opts = opts or {}
    ...
    detach_tab = function (w, view, blank_last)
        ...
        if blank_last ~= false and w.tabs:count() == 0 then
            w:new_tab_orig("luakit://newtab/", false)
        end
    end,
    ...
    navigate = function (w, arg, view)
        view = view or w.view
        if not view then return w:new_tab_orig(arg) end
...
function window.new(args)
    ...
    if w.tabs:count() == 0 then
        w:new_tab_orig(w:search_open(globals.homepage), false)
    end
